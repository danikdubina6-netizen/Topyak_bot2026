...
# Защита от потери токена: если на Render не подтянется переменная, возьмет этот токен автоматически
...
def send_welcome(message):
  bot.reply_to(message, f"Ку! Я в облаке Render. База: {len(PHRASES)} фраз.")
...
# Создаем Flask-приложение для поддержания активности порта
app = Flask(__name__)
