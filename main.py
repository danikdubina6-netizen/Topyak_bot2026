...
# Защита токена: берет из переменных окружения Render или использует запасной вариант
...
# Обработчик команды /start с твоим новым текстом
@bot.message_handler(commands=["start"])
def send_welcome(message):
  bot.reply_to(
      message, "Готовьтесь пообщаться с Топякским ботом и развлекайтесь рандомно!)"
  )


# Обработчик сообщений (отвечает, если бот упомянут или ему ответили реплаем)
...
# Настройка Flask-сервера для удержания порта на Render
app = Flask(__name__)
